...
export class VideosService {
  ...
  async incrementView(id: string): Promise<{ viewCount: number }> {
    ...
    // (e.g. 6, derived from likes+dislikes) with the raw column (1),
    // creating a visible discrepancy between body and meta description.
    const c = (await this.reactionsService.countsFor([id])).get(id) ?? {
      likes: 0,
      dislikes: 0,
    };
